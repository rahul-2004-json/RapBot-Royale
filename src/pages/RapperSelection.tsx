    ...
    <div className="min-h-screen bg-battle-dark battle-grid flex flex-col px-4 md:px-10 py-8">
          ...
          >
            {rappers.map((rapper) => (
              <motion.div
                ...
                className={`rapper-card ${selectedRapperA === rapper.id ? 'selected' : ''}`}
              >
                <div className="flex flex-col items-center">
                  <div className="w-24 h-24 rounded-full overflow-hidden mb-3 border-2 border-neutral-700">
                    <img src={rapper.image} alt={rapper.name} className="w-full h-full object-cover" />
                  </div>
                  <h3 className="text-lg font-battle text-white mb-1">{rapper.name}</h3>
                  <p className="text-sm text-neutral-400 text-center">{rapper.description}</p>
                </div>
              </motion.div>
            ))}
          </motion.div>
          ...
          >
            {rappers.map((rapper) => (
              <motion.div
                ...
                className={`rapper-card ${selectedRapperB === rapper.id ? 'selected' : ''}`}
              >
                <div className="flex flex-col items-center">
                  <div className="w-24 h-24 rounded-full overflow-hidden mb-3 border-2 border-neutral-700">
                    <img src={rapper.image} alt={rapper.name} className="w-full h-full object-cover" />
      ...
      <div className="mt-auto flex justify-center">
        <motion.button
          ...
          className={`flex items-center neon-button ${(!selectedRapperA || !selectedRapperB) ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
          <span>Continue</span>
